<?php 
    ...
    $archivos_facturas_ver = array_search('archivos_facturas_ver', array_column($permisos, 'permiso_variable'));
    if (empty($archivos_facturas_ver) && ($archivos_facturas_ver !== 0)) {
        header("location:../_errores/401.php");
    } else {

    $pagina = "Archivo"; 
    $factura_id_archivo = isset($_GET["factura_id_archivo"])?$_GET["factura_id_archivo"]:'';
?>
    ...
    <form id="upload_form" enctype="multipart/form-data" method="post">
        <input type="hidden" id="factura_id_archivo" name="factura_id_archivo" value="<?=$factura_id_archivo?>">
        <?php
        $archivos_facturas_alta = array_search('archivos_facturas_alta', array_column($permisos, 'permiso_variable'));
        if (!empty($archivos_facturas_alta) || ($archivos_facturas_alta === 0)) { 
        ?>
