...
$factura_id_archivo = isset($_POST["factura_id_archivo"])?$_POST["factura_id_archivo"]:'';
...
switch($opcion){
    ...
    case 'grilla_listar':
        grilla_listar($factura_id_archivo, $permisos);
        break;
...
function grilla_listar($factura_id_archivo, $permisos){
    

    $archivos = Archivo::listar_facturas($factura_id_archivo);
    ...
    $archivos_facturas_borrar = array_search('archivos_facturas_borrar', array_column($permisos, 'permiso_variable'));
    if (!empty($archivos_facturas_borrar) || ($archivos_facturas_borrar === 0)) {
