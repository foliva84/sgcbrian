<?php 
    ...
    $archivos_ver = array_search('archivos_ver', array_column($permisos, 'permiso_variable'));
    if (empty($archivos_ver) && ($archivos_ver !== 0)) {
        header("location:../_errores/401.php");
    } else {

    $pagina = "Archivo"; 
    $caso_id_archivo = isset($_GET["caso_id"])?$_GET["caso_id"]:'';
?>
    ...
    <form id="upload_form" enctype="multipart/form-data" method="post">
        <input type="hidden" id="caso_id_archivo" name="caso_id_archivo" value="<?=$caso_id_archivo?>">
        <?php
        $archivos_alta = array_search('archivos_alta', array_column($permisos, 'permiso_variable'));
        if (!empty($archivos_alta) || ($archivos_alta === 0)) { 
        ?>
