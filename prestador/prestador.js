...
$().ready(function(){
    ...
    // Date Picker
    $('#prestador_inicioActividades_n').datepicker();
    $('#prestador_inicioActividades').datepicker();
...
$("#formulario_modificacion").validate({
    ...
    rules:{
        prestador_nombre: {
            ...
            minlength: 4,
            remote: {
                url: "prestador_cb.php",
                type: "post",
                data: {
                    prestador_id: function() {
                        return $( "#prestador_id" ).val();
                    },
                    opcion: 'prestador_existe_modificacion'
                }
            }            
        },
    ...
    messages:{
        ...
        prestador_tipoPrestador_id: {
            required: "Por favor ingrese un tipo de prestador"
        },
        prestador_prestadorPrioridad_id: {
            required: "Por favor ingrese la prioridad del prestador"
        },
        prestador_direccion: {
...
// 
// Tipos de prestadores para el Formulario de Alta de Prestador
function formulario_alta_tipoPrestador(){

    var parametros = {
        "opcion": 'formulario_alta_tipoPrestador'
    };

    var miselect = $("#prestador_tipoPrestador_id_n");
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            miselect.append('<option value="">Seleccione un tipo de Prestador</option>');
            miselect_b.append('<option value="">Seleccione un tipo de Prestador</option>');

            for (var i=0; i<data.length; i++) {
                miselect.append('<option value="' + data[i].tipoPrestador_id + '">' + data[i].tipoPrestador_nombre + '</option>');
...
// Tipos de prestadores para el Formulario de Modificacion de Prestador
function formulario_modificacion_tipoPrestador(prestador_id){
   
    var parametros = {
        "opcion": 'formulario_modificacion_tipoPrestador',
        "prestador_id" : prestador_id
    };
    
    var miselect = $("#prestador_tipoPrestador_id");
    miselect.empty();
...
// Prioridades de prestador para el Formulario de Alta de Prestador
function formulario_alta_prestadorPrioridad(){

    var parametros = {
        "opcion": 'formulario_alta_prestadorPrioridad'
    };

    var miselect = $("#prestador_prestadorPrioridad_id_n");
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            for (var i=0; i<data.length; i++) {
                miselect.append('<option value="' + data[i].prestadorPrioridad_id + '">' + data[i].prestadorPrioridad_nombre + '</option>');
            }
        },
        error: function (xhr, ajaxOptions, thrownError) {
            $("#avisos").html('<div style="text-align:center; font-size:14px; margin-top:50px;">Existió un error.</div><div style="text-align:center; font-size:14px; margin-top:50px;">Mensaje para desarrollo= '+xhr.responseText+'</div>');
...
// Prioridades de prestador para el Formulario de Modificacion de Prestador
function formulario_modificacion_prestadorPrioridad(prestador_id){
   
    var parametros = {
        "opcion": 'formulario_modificacion_prestadorPrioridad',
        "prestador_id" : prestador_id
    };
    
    var miselect = $("#prestador_prestadorPrioridad_id");
    miselect.empty();
...
function formulario_alta_paises(){
    ...
    var miselect = $("#prestador_pais_id_n");
...
// Esto sirve para que cuando se modifica el select de país se borre lo que está en el input de ciudad
    $("#prestador_pais_id_n").change(function () {    
           $("#prestador_ciudad_id_n").val("");
           $("#prestador_ciudad_id_n_2").val("");
    });

    $('#prestador_ciudad_id_n').autocomplete({  
        source: function( request, response ) {
            $.ajax({
                ...
                data: {
                   ...
                   pais_id: $("#prestador_pais_id_n option:selected").val()
                },
                success: function( data ) {
		...
		$('#prestador_ciudad_id_n_2').val(names[1]);
...
// Paises para el Formulario de Modificacion de Prestador
function formulario_modificacion_paises(prestador_id){
    ...
    var miselect = $("#prestador_pais_id");
    miselect.empty();
 ...
 // Select Dependiente de Paises > Ciudades para el Formulario de Modificacion de Prestador
 // Esto sirve para que cuando se modifica el select de país se borre lo que está en el input de ciudad
    $("#prestador_pais_id").change(function () {    
           $("#prestador_ciudad_id").val("");
           $("#prestador_ciudad_id_2").val("");
    });


    $('#prestador_ciudad_id').autocomplete({  
        source: function( request, response ) {
            $.ajax({
                method: "post",
                url : 'prestador_cb.php',
                ...
                data: {
                    ciudad: request.term,
                    opcion: 'select_ciudades',
                    pais_id: $("#prestador_pais_id option:selected").val()
                },
                success: function( data ) {
                    response( $.map( data, function( item ) {
                        var code = item.split("|");
        ...
        select: function( event, ui ) {
            var names = ui.item.data.split("|");						
            $('#prestador_ciudad_id_2').val(names[1]);	
	}		      	
    });


// Contratos de prestador para el Formulario de Alta de Prestador
function formulario_alta_prestadorContrato(){

    var parametros = {
        "opcion": 'formulario_alta_prestadorContrato'
    };

    var miselect = $("#prestador_contrato_id_n");
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            miselect.append('<option value="">Seleccione estado contractual</option>');
            
            for (var i=0; i<data.length; i++) {
                miselect.append('<option value="' + data[i].prestadorContrato_id + '">' + data[i].prestadorContrato_nombre + '</option>');
            }
        },
        error: function (xhr, ajaxOptions, thrownError) {
            $("#avisos").html('<div style="text-align:center; font-size:14px; margin-top:50px;">Existió un error.</div><div style="text-align:center; font-size:14px; margin-top:50px;">Mensaje para desarrollo= '+xhr.responseText+'</div>');
...
// Contratos de prestador para el Formulario de Modificacion de Prestador
function formulario_modificacion_prestadorContrato(prestador_id){

    var parametros = {
        "opcion": 'formulario_modificacion_prestadorContrato',
        "prestador_id" : prestador_id
    };

    var miselect = $("#prestador_contrato_id");
    miselect.empty();
...
var agrega_prestador_formulario = function(){
...
var modal_baja = function($prestador_id){
   $('#ventana_modal_borrado').modal('show');
   $('#prestador_id_b').val($prestador_id);
};


var modal_alta = function($prestador_id){
   $('#ventana_modal_habilita').modal('show');
   $('#prestador_id_a').val($prestador_id);
   
};


//Desactiva un prestador - borrado lógico
$( "#formulario_baja" ).on( "submit", function(e) {
    
    e.preventDefault();
  
    var prestador_id_b = $('#prestador_id_b').val();
  
    var parametros = {
        "prestador_id_b": prestador_id_b,
    ...
    $.ajax({
        ...
        success: function (resultado) {
            if(resultado=='ok'){
                $.Notification.autoHideNotify('success', 'top right', 'Deshabilitado exitosamente...','Se ha deshabilitado la prestador.');
                grilla_listar('','');
...
$( "#formulario_habilita" ).on( "submit", function(e) {
    ...
    var prestador_id_a = $('#prestador_id_a').val();
  
    var parametros = {
        "prestador_id_a": prestador_id_a,
    ...
    $.ajax({
            ...
            success: function (resultado) {
                if(resultado==='ok'){
                    $.Notification.autoHideNotify('success', 'top right', 'Habilitado exitosamente...','Se ha rehabilitado el prestador.');
                    grilla_listar('','');
...
// Carga los datos para mostrar el formulario de vista del prestador
function formulario_lectura(prestador_id){
    
    // scrollea la pagina hasta arriba al cargar la funcion
    window.scrollTo(0,0);
    
    // carga el prestador_id en los elementos object
    $('#pantalla_territorialidad_v').attr('data','prestador_ciudades.php?prestador_id=' + prestador_id + '&prestador_vista=1');
    $('#pantalla_practica_v').attr('data','prestador_practica.php?prestador_id=' + prestador_id + '&prestador_vista=1');
    $('#pantalla_archivos_v').attr('data','../archivos_prestadores/archivo.php?prestador_id=' + prestador_id + '&prestador_vista=1');
    ...
    var parametros = {
     ...
     $.ajax({
        ...
        data: parametros,
        success:function(data){
            $('#prestador_id_v').val(data.prestador_id);
            $('#prestador_nombre_v').val(data.prestador_nombre);
            $('#prestador_razonSocial_v').val(data.prestador_razonSocial);
            $('#prestador_paginaWeb_v').val(data.prestador_paginaWeb);
            $('#prestador_direccion_v').val(data.prestador_direccion);
            $('#prestador_codigoPostal_v').val(data.prestador_codigoPostal);
            $('#prestador_contratoObservaciones_v').val(data.prestador_contratoObservaciones);
            $('#prestador_bancoIntermediario_v').val(data.prestador_bancoIntermediario);
            $('#prestador_bancoBeneficiario_v').val(data.prestador_bancoBeneficiario);
            $('#prestador_taxID_v').val(data.prestador_taxID);
            $('#prestador_inicioActividades_v').val(data.prestador_inicioActividades);
            $('#prestador_observaciones_v').val(data.prestador_observaciones);
                       
            $('#prestador_tipoPrestador_nombre_v').val(data.tipoPrestador_nombre);
            $('#prestador_prestadorPrioridad_nombre_v').val(data.prestadorPrioridad_nombre);
            $('#prestador_pais_nombre_v').val(data.pais_nombreEspanol);
            $('#prestador_contrato_nombre_v').val(data.prestadorContrato_nombre);
            
            grilla_telefonos_v();
            grilla_emails_v();
            
            $('#prestador_ciudad_nombre_v').val(data.ciudad_nombre);
            $('#prestador_ciudad_id_2').val(data.prestador_ciudad_id);
...
// Carga los datos para mostrar el FORMULARIO MODIFICAR de prestador
$("#btn_modificar_prestador_footer").on('click', function() {
    
    // scrollea la pagina hasta arriba al cargar la funcion
    window.scrollTo(0,0);
    
    let prestador_id = $('#prestador_id_v').val();
 
    // carga el prestador_id en los elementos object
    $('#pantalla_territorialidad').attr('data','prestador_ciudades.php?prestador_id=' + prestador_id);
    $('#pantalla_practica').attr('data','prestador_practica.php?prestador_id=' + prestador_id);
    $('#pantalla_archivos').attr('data','../archivos_prestadores/archivo.php?prestador_id=' + prestador_id);
    ...
    var parametros = {
     ...
     $.ajax({
        ...
        success:function(data){
            ...
            $('#prestador_nombre').val(data.prestador_nombre);
            $('#prestador_razonSocial').val(data.prestador_razonSocial);
            $('#prestador_paginaWeb').val(data.prestador_paginaWeb);
            $('#prestador_direccion').val(data.prestador_direccion);
            $('#prestador_codigoPostal').val(data.prestador_codigoPostal);
            $('#prestador_contratoObservaciones').val(data.prestador_contratoObservaciones);
            $('#prestador_bancoIntermediario').val(data.prestador_bancoIntermediario);
            $('#prestador_bancoBeneficiario').val(data.prestador_bancoBeneficiario);
            $('#prestador_taxID').val(data.prestador_taxID);
            $('#prestador_inicioActividades').val(data.prestador_inicioActividades);
            $('#prestador_observaciones').val(data.prestador_observaciones);
            ...
            $('#prestador_ciudad_id').val(data.ciudad_nombre);
            $('#prestador_ciudad_id_2').val(data.prestador_ciudad_id);
...
// Lista de Telefonos para el Formulario de Alta de Prestador
function listarTipoTelefonos_prestador(){

    var parametros = {
        "opcion": 'listarTipoTelefonos_prestador'
    };
...
var telefono_guardar = function(){
    ...
    if (telefonoTipo_id == '' || telefono_numero == ''){
        $.Notification.autoHideNotify('error', 'top cneter', 'Debe completar todos los campos ', 'Algunos de los campos no estan completos.');
        return;
    } 
...
var telefono_editar = function(telefono_id_e){
    ...
    $.ajax({
            ...
            success: function (data) {
                ...
                $('#telefonoTipo_id').css("border", "1px solid #5882FA");
...
// Lista de tipos de email para el Formulario de Alta de Prestador
function listarTipoEmail_prestador(){

    var parametros = {
        "opcion": 'listarTipoEmail_prestador'
    };
...
var email_editar = function(email_id_e){
    ...
    // $.Notification.autoHideNotify('success', 'top right', 'E-mail principal...',email_principal);
...
var listar_prestadores = function(){
