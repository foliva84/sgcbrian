...
function formulario_lectura(factura_id) {
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            $('#fci_numero_factura_v').val(data.numeroFactura);
...
$("#formulario_autorizacion_fci").validate({  
    ...
    rules: {
        ...
        fci_importeAprobadoOrigen_auto: {
            required: function(e){
                return $("#fci_estado_au_id").val() === "1";
            },
            maxStrict: function(e){
                return $("#fci_importeRechazadoOrigen_auto").val();
            },
            min: 0
        },
    ...
    submitHandler: function (form) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $.Notification.autoHideNotify('success', 'top center', 'El ITEM de la Factura se actualizó exitosamente...', 'Los cambios han sido guardados.');
                
                // Arma la grilla con los ITEMS de Factura
                grilla_fcItems();

                // Actualiza las grillas: mostrar_op + datos_op -> fci_orden_pago.js
                console.log('data: ' + data);

                mostrar_op(data); // toFix2 - Resolver la forma en como obtiene op_id
                datos_op(data); // toFix2 - Resolver la forma en como obtiene op_id
...
// Para evitar el error cuando se usa la función 'formulario_autorizacion_fci' desde Facturas, se declaran estas dos funciones vacias. Estas mismas estan declaradas en 'fci_orden_pago.js'
//function mostrar_op() {}
//function datos_op() {}
...
function seleccion_servicios(caso_id) {
    ...
    $("#btn_cargarServicios").on('click', function() {
        ...
        let pagador         = $('#caso_pagador_v').val();
        let factura_id      = $('#factura_id_v').val();
...
let autorizar_fci = function(fci_id, op_id, tipo_form) {
    ...
    $.ajax({
        ...
        success:function(data) {
            ...
            // Formatea los campos de monedas
            $('#fci_importeOrigen_au').number( true, 2, ',', '.' );
            $('#fci_importeAprobadoOrigen_au').number( true, 2, ',', '.' );
            $('#fci_importeOrigen_auto').number( true, 2, ',', '.' );

            // Si el modal se llama con el btn de una OP (tipo_form = 1), no se muestra la info de un FCI
            if (tipo_form == 1) {
                $('#info_fci_au').addClass('hidden');
            } else {
                $('#info_fci_au').removeClass('hidden');
            }
            ...
            $('#fci_importeOrigen_au').val(data.importeOrigenFci);
            $('#fci_importeAprobadoOrigen_au').val(data.importeAprobadoOrigen);
            $('#fci_importeOrigen_auto').val(data.importeOrigenFci);
...
$("#fci_importeAprobadoOrigen_auto").on('keyup', function() {
    
    let importe_real_Origen = $('#fci_importeOrigen_auto').val();
    let importe_aprobado    = $('#fci_importeAprobadoOrigen_auto').val();
    let importe_rechazado   = (importe_real_Origen - importe_aprobado);
    
    $('#fci_importeRechazadoOrigen_auto').val(importe_rechazado);
    
    if (importe_rechazado == 0)  {
        $('#fci_mov_auditoria_auto_id').val(2); // 2 - Item Aprobado por Facturación
    } else if (importe_real_Origen == importe_rechazado) {
        $('#fci_mov_auditoria_auto_id').val(4); // 4 - Item Rechazado por Facturación
    } else if (importe_rechazado < 0) {
        $('#fci_mov_auditoria_auto_id').val(10); // 10 - Seleccione (En caso que el 'Valor Aprobado' sea superior al 'Valor Real Origen')
    } else {
...
let preparar_formulario_altaFci = function() {
    ...
    $('#fci_importeOrigen_v').number( true, 2, ',', '.' );
    ...
    // Deshabilita los campos de importe que no debe modificar el usuario
    $('#fci_importeOrigen_v').prop("disabled", true);
    $('#fci_tipoCambio_n').prop("disabled", true);
    $('#fci_importeUSD_n').prop("disabled", true);
...
let preparar_formulario_autorizacion_fci = function(estado_fci) {
    ...
    // Formatea los campos de monedas
    $('#fci_importeAprobadoOrigen_auto').number( true, 2, ',', '.' );
    $('#fci_importeRechazadoOrigen_auto').number( true, 2, ',', '.' );
...
// Prepara el campo para mostrar la asociación de FCIs
let asociar_fci = function(fci_id) {
    $('#fci_asociado_id_n').val(fci_id);
    $('#div_fci_asociado').removeClass('hidden');
};
...
// Selecciona el Estado para el Formulario de Autorizacion de Items
    ...
    $.ajax({
        ...
        success:function(data) {
            ...
            if (form === 'alta') {
                ...
                $.Notification.autoHideNotify('error', 'top center', 'Error al convertir la moneda');
            }
        }
    });
}

// Calcula el Importe total de Origen
$('#fci_imp_medicoOrigen_n').add('#fci_imp_feeOrigen_n').add('#fci_descuento_n').add('#fci_tipoCambio_n').on('keyup', function() {
    ...
    $('#fci_importeOrigen_v').val(importe_origen);
});
...
$("#btn_listar_servicios").on('click', function() {
    ...
    grilla_listar_servicios(caso_numero, prestador_id);
    mostrar_pagador(caso_numero);
});

// Lista los Servicios del Caso Seleccionado en la carga del FCI
let grilla_listar_servicios = function(caso_numero_buscar, prestador_id){
...
// Lista los Servicios del Caso Seleccionado en la carga del FCI
let mostrar_pagador = function(caso_numero){

    let parametros = {
        "caso_numero": caso_numero,
        "opcion": 'mostrar_pagador'
    };
    $.ajax({
        dataType: "JSON",
        method: "POST",
        url: "../cliente/cliente_cb.php",
        data: parametros,
        success:function(data) {
            $("#caso_pagador_v").val(data);
        }
    });
...
let grilla_fcItems = function(){
       
    let factura_id = $("#factura_id_v").val();
    
    let parametros = {
        "tipo_consulta": 'grilla_fcItems',
        "factura_id": factura_id,
...
// Busca los datos para mostrar los Servicios relacionados con los FCI
...
$("#btn_cancelar_nuevo_fci").click(function() {
    ...
    $('#div_fci_asociado').addClass('hidden');
});
